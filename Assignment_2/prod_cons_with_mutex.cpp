...
sem_t empty_semaphore;	// count of empty slots
sem_t full_semaphore;	// count of full buffer slots
queue<int> q;
...
{	
	...
	for(i=1;i<=10;i++)
	{
		sem_wait(&empty_semaphore);		// down(empty)
		...
		sem_post(&full_semaphore);		// up(full)
	}
}
