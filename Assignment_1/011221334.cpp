...
void sjf_preemptive(vector<Process> processes)
{
    ...
    {
        //  fifo is both remTime are equal
        if (a->remaining_time == b->remaining_time)
            return a->arrival_time > b->arrival_time;
        return a->remaining_time > b->remaining_time; // lower remaining time = higher priority
    };
    ...
    while (completed < n)
    {
        ...
        {
            pq.push(current); // if still has remaining time
        }
        else
        {
...
void preemptivePriority(vector<Process> processes)
{
    ...
    auto cmp = [](Process *a, Process *b)
    {
        if (a->priority == b->priority)
            return a->arrival_time > b->arrival_time;
        return a->priority > b->priority; // lower remaining time = higher priority
    };
